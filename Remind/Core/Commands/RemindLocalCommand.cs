...
{
    public class RemindLocalCommand : IChatCommand
    {
        public const string CMD = "remindlocal";
        public string Name => CMD;
        public string ShortName => "rl";
        public string Description => $"Send a local chat message after a delay or at a time. Usage: /{CMD} [in hh:mm:ss | at HH:mm] [message]";
        ...
        public void Execute(string[] args)
        {
            ...
            if (args[0] == "in")
            {
                if (!RemindPlugin.ScheduledTaskManager.TryScheduleIn(args[1], () =>
                    ChatUtils.SendMessageAsync(RemindPlugin.ModName, message, Islocal: true), out _, out string error))
                {
